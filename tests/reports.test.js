...
beforeAll(async () => {
	...
	const { body: projectResponse } = await api
		.post('/projects')
		.set('Authorization', `Bearer ${token}`)
		.send(projectToInit);

	projectToInit = projectResponse;

	reportExample.project = projectToInit.id;
	reportExample.week = projectToInit.startWeek;
});
...
afterAll(async () => {
	await api
		.delete(`/projects/${projectToInit.id}`)
		.set('Authorization', `Bearer ${token}`);
